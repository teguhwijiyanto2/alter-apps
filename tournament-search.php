  ...
  <body>
    <div class="position-relative container">
      <div class="w-100 pt-4">
        <!-- Top Bar Start -->
		<form action="tournament-search.php" method="POST">
          ...
          >
			...
			<?php
				if(isset($_POST['q']) && trim($_POST['q']) !== "") {
					...
					$default_txt = "placeholder='Search for tournaments'";
				}
			?>
            <input
		...
		<?php
		$results_2 = DB::query("SELECT * FROM tournament where name like '%".$_POST['q']."%' OR description like '%".$_POST['q']."%' order by date_from desc");
		foreach ($results_2 as $row_2) {
			...
			echo "
			  <div class='d-flex flex-column gap-2 mt-3' style='cursor:pointer;' onclick=\"window.location.href='tournament-detail.php?tid=".$row_2['id']."';\">
				<div class='bg-dark rounded-3 overflow-hidden w-100'>
				  <div
					class='position-relative p-3 pt-5 bg-secondary bg-opacity-50'
					style=\"
					  background-image: url('tournament_banner/".$row_2['banner']."');
				  ...
				  >
					...
					<div class='d-flex flex-row align-items-center gap-3 mt-4'>
					  <img
						src='tournament_thumbnail/".$row_2['thumbnail']."'

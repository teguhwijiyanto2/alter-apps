        ...
        > 
		  ...
		  <?php
			  if(isset($_POST['q']) && trim($_POST['q'])!=="") {
				  ...
				  $default_txt = "placeholder='Search for Chat Room'";
			  }	
		  ?>
...
*/
	...
	if(isset($_POST['q']) && trim($_POST['q'])!=="") {
			...
			unset($array_chat_room_uuid_shown);			
			$array_chat_room_uuid_shown = array();		  
			...
			foreach ($results_1 as $row_1) {
					...
					if($row_1['user_id_1']==$_SESSION["session_usr_id"]) {
						...
						$user_id_chat_with = $row_1['user_id_2'];
					}
					else {
						$name_list = $array_users_name[$row_1['user_id_1']];
						$user_id_chat_with = $row_1['user_id_1'];
					}
					
					$user_pp_file_name = DB::queryFirstField("select user_pp_file from users where id=%i", $user_id_chat_with);
					$name_shown = $name_list;
					$message_shown = $row_1['last_message'];				
							
					$count_message = DB::queryFirstField("select count(*) from chat where chat_room_uuid=%s AND message like '%".$_POST['q']."%' order by created_on desc limit 0,1", $row_1['chat_room_uuid']);														
					if($count_message > 0) {
						$message_shown = str_ireplace($_POST['q'], "<b>".$_POST['q']."</b>", $row_1['last_message']);
						$message_shown = "...".$message_shown."...";
					} // if($count_message > 0) {
												
					$count_user = DB::queryFirstField("select count(*) from users where id=%i AND name like '%".$_POST['q']."%'", $user_id_chat_with);
					if($count_user > 0) {
						$name_shown = str_ireplace($_POST['q'], "<b>".$_POST['q']."</b>", $name_list);
					} // if($count_message > 0) {						

					if($count_message > 0 || $count_user > 0) {
						
						if (in_array($row_1['chat_room_uuid'], $array_chat_room_uuid_shown)){
						  // nothing to do
						  //echo "aaa";
						} // if (in_array($row_1['chat_room_uuid'], $array_chat_room_uuid_shown)){
							
						else{ // chat room itu belum ditampilkan, maka tampilkan di list chat						
							...
							<a aria-label='Chat Item' href='#' onclick=\"document.getElementById('formChatOpener_".$row_1['chat_room_uuid']."').submit();\">
							  ...
							  >
								...
								/>
								<div class='flex-fill'>
								  <h6 class='mb-0'>".$name_shown."</h6>
								  <span class='lh-1 text-secondary'
									><small>".$message_shown."</small>
								  </span>
								</div>
								<div
							...
							$array_chat_room_uuid_shown[] = $row_1['chat_room_uuid'];
		...
		/* // utk check aja
		foreach($array_chat_room_uuid_shown as $key => $val) {
			echo "$key => $val<br>";
		}
		*/	
